...
(defmacro with-server-options ((options server-options) &body body)
  `(cffi:with-foreign-objects ((,options '(:struct world-options))
			       (device-name :char 1024))
     (let* ((device (server-options-device ,server-options)))
       (when device
	 (cffi:lisp-string-to-foreign device device-name 1024))
       (set-slots (,options)
	 ...
	 in-device-name (if device device-name  (cffi-sys:null-pointer))
	 ...
	 out-device-name (if device device-name (cffi-sys:null-pointer))
